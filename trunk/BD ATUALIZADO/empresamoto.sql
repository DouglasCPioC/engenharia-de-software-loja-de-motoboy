...
--
-- Máquina: 127.0.0.1
-- Data de Criação: 21-Jan-2014 às 20:58
...
CREATE TABLE IF NOT EXISTS `ordemserv` (
  ...
  `codbairro` bigint(20) DEFAULT NULL,
  `codcli` bigint(20) DEFAULT NULL,
  `codtipo` bigint(20) DEFAULT NULL,
  `numsai` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`numord`),
  KEY `codat` (`codat`,`codbairro`,`codcli`,`codtipo`,`numsai`),
  KEY `codtipo` (`codtipo`),
  KEY `numsai` (`numsai`),
  KEY `codreg` (`codbairro`),
  KEY `codcli` (`codcli`)
...
INSERT INTO `ordemserv` (`numord`, `dataemissao`, `datalimite`, `descr`, `total`, `statusos`, `codat`, `codbairro`, `codcli`, `codtipo`, `numsai`) VALUES
...
ALTER TABLE `ordemserv`
  ADD CONSTRAINT `ordemserv_ibfk_6` FOREIGN KEY (`codbairro`) REFERENCES `bairro` (`codbairro`) ON DELETE SET NULL ON UPDATE CASCADE,
